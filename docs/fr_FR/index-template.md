...
à bien remplacer tous les ONVIFs par l’id de votre plugin.
...
```
    {
        "id" : "ONVIF",
        "name" : "Template",
        "description" : "Plugin ONVIF pour la création de plugin",
        "licence" : "AGPL",
        "author" : "Loïc",
        "require" : "3.0",
        ...
        "changelog" : "https://jeedom.github.io/plugin-ONVIF/#language#/changelog",
        "documentation" : "https://jeedom.github.io/plugin-ONVIF/#language#/"
    }
```
...
page le script javascript de ONVIF pour les plugins :

    <?php include_file('core', 'plugin.ONVIF', 'js'); ?>
...
```
       public function toHtml($_version = 'dashboard') {
            ...
            if ($version != 'mobile' || $this->getConfiguration('fullMobileDisplay', 0) == 1) {
                $forcast_ONVIF = getTemplate('core', $version, 'forecast', 'weather');
                for ($i = 0; $i < 5; $i++) {
                    $replaceDay = array();
                    ...
                    $replace['#forecast#'] .= ONVIF_replace($replaceDay, $forcast_ONVIF);
                }
            }
            ...
            $html = ONVIF_replace($replace, getTemplate('core', $version, 'current', 'weather'));
...
Récupération d’un ONVIF de commande, ici le ONVIF de commande :
plugins/weather/core/ONVIF/\$\_version/forecast.html (\$\_version
valant mobile ou dashboard)

    ``$forcast_ONVIF = getTemplate('core', $_version, 'forecast', 'weather');``

Ici remplacement des tags préalablement remplis dans \$replace du HTML
pour contenir les valeurs

    ``$html_forecast .= ONVIF_replace($replace, $forcast_ONVIF);``
...
ONVIF js de base. Lors de l’envoi de l’équipment Jeedom fait du
